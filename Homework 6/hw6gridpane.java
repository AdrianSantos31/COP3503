...
public class hw6gridpane extends Application{
  ...
  public void start(Stage primaryStage) throws Exception{
    ...
    gridPane.add(clock, 1, 0);
    gridPane.add(hangmanPane, 2, 0);

    //Create a scene and place it in the Stage
    Scene scene = new Scene(gridPane,720,330);
    ...
    clock.setW(gridPane.getWidth()));
    gridPane.heightProperty().addListener(ov ->
    clock.setH(gridPane.getHeight()));*/
    ...
    for (int i=0; i < 2; i++){
      for (int j = 0; j < 2; j++){
        ...
        //fan.centerXProperty().bind(fanPane.widthProperty().divide(2));
        //fan.centerYProperty().bind(fanPane.heightProperty().divide(2));
        fan.setCenterX(x);
        fan.setCenterY(y);
